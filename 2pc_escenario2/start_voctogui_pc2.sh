...
# IP of PC 1 (server). Override if the network address changes:
#   IP_CEREBRO=10.0.0.5 ./start_voctogui_pc2.sh
...
if ! wait_for_connection "$IP_CEREBRO" 9999; then
    log "Server not responding at $IP_CEREBRO:9999. Is PC 1 running?"
    exit 1
fi
...
PYTHONWARNINGS="ignore::DeprecationWarning" python3 voctogui.py -H "$IP_CEREBRO"
