...
class Voctogui(object):
    ...
    def __init__(self):
        ...
        # ============================================================
        # Optional GUI state exporter for the telemetry service
        # Safe: failures here do not affect the GUI
        # ============================================================
        try:
            from lib.gui_state_exporter import GuiStateExporter
            self.gui_state_exporter = GuiStateExporter(self.ui, interval=1.0)
            self.gui_state_exporter.start()
            self.log.info('GuiStateExporter started successfully')
        except Exception as e:
            self.log.warning('GuiStateExporter could not be started: %s', e)
    ...
    def quit(self):
        ...
        if self.gui_state_exporter is not None:
            try:
                self.gui_state_exporter.stop()
                self.log.info('GuiStateExporter stopped')
            except Exception as e:
                self.log.warning('GuiStateExporter could not be stopped cleanly: %s', e)
