...
class VideoPreviewsController(object):
    ...
    def __init__(self, preview_box, win, uibuilder):
        ...
        for idx, source in enumerate(self.sources):
            # Hide preview panels for utility sources that are not operator-selectable
            if source in ['break', 'intro']:
                self.log.info('Hiding preview for source: %s', source)
                continue
    ...
    def slider_changed(self, slider):
        ...
        Connection.cam_volumes[source] = volume
        if not Connection.is_blanked:
    ...
    def on_audio_status(self, *volumes):
        ...
        for source, volume in volumes.items():
            # Hidden sources (break, intro) have no slider; skip them
            if source not in self.volume_sliders:
                continue
