...
class VocConfigParser(SafeConfigParser):
    ...
    def getShowFullScreenButton(self):
        return self.getboolean('misc', 'fullscreen', fallback=True)

    def hasOverlay(self):
        """Return True if the [overlay] section is present (inherited from server config)."""
        return self.has_section('overlay')

    def getOverlayAutoOff(self):
        """Return the default state of the AUTO-OFF button."""
