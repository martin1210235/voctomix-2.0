...
class ControlServerCommands(object):
    ...
    def get_sources_status(self):
        """returns the connection status of each input source.
        true = source is actively connected (signal present),
        false = no client connected to that port."""
        result = {}
        for name, source in zip(self.sources, self.pipeline.sources):
            if hasattr(source, 'currentConnection'):
                result[name] = source.currentConnection is not None
            else:
                # Non-TCP sources (static images, test patterns) are always considered active
                result[name] = True
        return OkResponse('sources_status', json.dumps(result))
    ...
    if Config.hasOverlay():
        ...
        def get_overlays_title(self):
            ...
            title_list = Config.getOverlaysTitle() or []
            return NotifyResponse('overlays_title',
                                  ",".join(quote(t) for t in title_list))

        def get_overlays(self):
            """respond with list of all available overlays"""
            files = Config.getOverlayFiles() or []
            return NotifyResponse('overlays',
                                  ",".join([quote(a) for a in files]))

    if Config.hasLogo1():
        def show_logo1(self, visible):
            self.pipeline.vmix.showLogo1(str2bool(visible))
            return self.get_logo1_visible()

        def get_logo1_visible(self):
            return NotifyResponse('logo1_visible', str(self.pipeline.vmix.getLogo1Visible()))

    if Config.hasLogo2():
        def show_logo2(self, visible):
            self.pipeline.vmix.showLogo2(str2bool(visible))
            return self.get_logo2_visible()

        def get_logo2_visible(self):
            return NotifyResponse('logo2_visible', str(self.pipeline.vmix.getLogo2Visible()))

    def set_text(self, *args):
        """Updates the live text overlay 1 in the GStreamer pipeline."""
        texto = dequote(" ".join(args))
        self.pipeline.vmix.set_dynamic_text(texto)
        return NotifyResponse('text_updated', quote(texto))

    def set_text2(self, *args):
        """Updates the live text overlay 2 in the GStreamer pipeline."""
        texto = dequote(" ".join(args))
        self.pipeline.vmix.set_dynamic_text_2(texto)
        return NotifyResponse('text2_updated', quote(texto))
