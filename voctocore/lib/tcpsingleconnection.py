...
class TCPSingleConnection(object, metaclass=ABCMeta):
    ...
    def on_connect(self, sock, *args):
        ...
        # Immediately detect when the client (ffmpeg) disconnects or crashes
    ...
    def _on_connection_dropped(self, conn, condition):
        self.log.info('Disconnect detected (HUP/ERR) — releasing source')
        if self.currentConnection is conn:
            self.disconnect()
