...
# Can be overridden: IP_LOCAL=10.0.0.5 ./start_server.sh
...
# Prevents "Address already in use" errors when running scenarios back to back
...
# 1. Launch voctocore (the core mixer)
log "Launching voctocore..."
cd "$BASE_DIR/voctocore"
python3 voctocore.py 2> >(grep -v "Broken pipe" >&2) &
