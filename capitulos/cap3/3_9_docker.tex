...
\begin{itemize}
  ...
  \item Librerías matemáticas \texttt{scipy} y \texttt{numpy}, necesarias para el
...
\textbf{\textit{Shared Network Namespace}}: los contenedores de las cámaras y el servicio de telemetría
...
\begin{itemize}
  ...
  \item \textbf{En el servicio de telemetría (Python):} la opción \texttt{SO\_REUSEADDR}
...
Para reducir el tiempo de despliegue y eliminar errores de configuración manual, se
desarrollaron dos scripts maestros de shell que unifican el arranque de todos los
componentes del sistema en el escenario nativo (sin Docker):

\begin{itemize}
  \item \textbf{\texttt{lanzar\_realizacion.sh}} (modo producción): arranca voctocore,
        inyecta las señales de continuidad (vídeos de pausa, música de fondo) y lanza
        voctogui. Concebido para sesiones reales sin fuentes de prueba sintéticas.
  \item \textbf{\texttt{pruebas\_lanzar\_realizacion.sh}} (modo laboratorio): además de
        la infraestructura anterior, inyecta automáticamente cuatro fuentes de prueba
        FFmpeg (cam1--cam4 con \textit{test card} y reloj sobreimpresionado), lo que
        permite probar transiciones, composites y rotulación sin cámaras físicas.
\end{itemize}

Ambos scripts implementan las siguientes garantías de robustez, equivalentes a las que
Docker Compose proporciona de forma nativa en el escenario contenedorizado:

\begin{itemize}
  \item \textbf{Espera activa de puertos:} un bucle consulta \texttt{ss -lnt} hasta
        confirmar que el puerto 9999 está en escucha, sustituyendo los \texttt{sleep}
        fijos del código original y eliminando las condiciones de carrera al arranque.
  \item \textbf{Auto-limpieza (\texttt{trap}):} al cerrar voctogui, la señal
        \texttt{SIGCHLD} dispara \texttt{kill} masivo de todos los procesos hijos,
        garantizando la liberación inmediata de los puertos de red.
  \item \textbf{Rutas dinámicas:} \texttt{BASE\_DIR="\$(cd "\$(dirname "\$0")" \&\& pwd)"}
        elimina todas las rutas absolutas \textit{hardcodeadas}, haciendo el proyecto
        completamente portátil a cualquier máquina sin modificación de código.
\end{itemize}
