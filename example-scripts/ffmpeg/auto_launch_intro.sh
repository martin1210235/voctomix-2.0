...
while true; do
    ...
    -f lavfi -i anullsrc=r=${AUDIORATE}:cl=stereo -pix_fmt yuv420p \
    -s "${WIDTH}x${HEIGHT}" -r "${FRAMERATE}" -c:v rawvideo -c:a pcm_s16le \
    -map 0:v:0 -map 1:a:0 -shortest -f matroska tcp://127.0.0.1:${INTRO_PORT}
