...
\medskip\noindent\textbf{Escenario de un PC.}
...
en el fichero de configuración de voctogui), y muestra en la interfaz todas las
fuentes activas. Al cerrar la interfaz, los procesos auxiliares se terminan automáticamente (figura~\ref{fig:escenario_1pc}).

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figuras/escenario_1pc.png}
  \caption{Escenario de despliegue nativo con un único PC.}
  \label{fig:escenario_1pc}
\end{figure}

\medskip\noindent\textbf{Escenario de dos PCs.}
En este escenario la carga de trabajo se reparte entre dos equipos conectados por
red local. El proceso de arranque es análogo al de un PC pero distribuido: en el
PC servidor se lanza \texttt{voctocore.py} y se inyectan todas las señales de
vídeo y audio mediante FFmpeg, exactamente igual que en el escenario anterior.
En el PC del realizador se lanza únicamente \texttt{voctogui.py}, que recibe como
parámetro la dirección IP del servidor (\texttt{IP\_SERVER}) para conectarse al
puerto de control~9999 a través de la red local, en lugar de \texttt{localhost}.
Adicionalmente, el PC del realizador abre un monitor de programa conectándose al
puerto de salida~15000 del servidor, lo que permite supervisar la señal de programa
en tiempo real desde el puesto del operador (figura~\ref{fig:escenario_2pc}).

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figuras/escenario_2pc.png}
  \caption{Escenario de despliegue nativo con dos PCs en red local.}
  \label{fig:escenario_2pc}
\end{figure}
