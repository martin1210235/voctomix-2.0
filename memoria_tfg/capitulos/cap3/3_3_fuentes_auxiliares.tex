...
\texttt{break} e \texttt{intro} son fuentes de vídeo pregrabadas que \texttt{voctocore} trata
...
\begin{itemize}
  ...
  \item \textbf{intro}: vídeo de reproducción única. Al asignar intro a cualquier
        canal del mezclador (A o B), \texttt{voctogui} detiene el proceso FFmpeg que alimenta
        esa entrada y lanza uno nuevo que reproduce el vídeo desde el principio sin
        bucle. Una vez que la nueva señal está estabilizada, la interfaz envía el
        comando de corte al núcleo para que la intro comience a emitirse desde el
        primer fotograma.
\end{itemize}
...
entrada acepta un flujo Matroska de solo audio (PCM S16LE, 48~kHz, estéreo) y su
nivel de salida puede controlarse desde el protocolo de control.

\subsection{\textit{stream blanker}: estados de emisión}

El \textit{stream blanker} es el mecanismo que permite interrumpir la señal de programa hacia el
...
entre la mezcla y el puerto de salida de \textit{streaming} (puerto 15000).

El sistema define tres estados de emisión, controlables desde la barra de herramientas
de \texttt{voctogui}:

\begin{itemize}
  \item \textbf{LIVE:} se emite la mezcla activa producida por \texttt{voctocore}. El realizador
  ...
  \item \textbf{NOSTREAM:} se emite un vídeo de ``fuera de emisión'' procedente del
        puerto 17001, con el audio de la mezcla en vivo bloqueado. Indica al espectador
...
\begin{figure}[H]
  ...
  \end{minipage}
  \caption{Estados NOSTREAM y PAUSE del \textit{stream blanker}: vista del realizador (arriba)
