...
\begin{enumerate}
  ...
  \item Al pulsar \textit{Enter} o el botón INSERT, voctogui envía el comando TCP
        \texttt{set\_text <texto>} al núcleo.
  \item voctocore llama a \texttt{set\_dynamic\_text()}, que actualiza directamente
        la propiedad \texttt{text} del elemento \texttt{textoverlay} del pipeline
        GStreamer, sin escritura a disco ni renderización externa.
  \item El texto aparece en antena en el siguiente fotograma, con la tipografía y
        posición configuradas en el pipeline.
\end{enumerate}
